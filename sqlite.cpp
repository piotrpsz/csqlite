...
bool sqlite_t::open(fs::path const &path, bool const read_only) noexcept {
    ...
    if (SQLITE_OK == sqlite3_open_v2(reinterpret_cast<const char *>(path.c_str()), &db_, flags, nullptr)) {
...
bool sqlite_t::create(fs::path const &path, function<bool(sqlite_t *)> const &lambda, bool override) noexcept {
    ...
    if (SQLITE_OK == sqlite3_open_v2(reinterpret_cast<const char *>(path.string().c_str()), &db_, flags, nullptr)) {
        if (!lambda(this)) {
